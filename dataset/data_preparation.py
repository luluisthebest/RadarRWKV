...
def radarcube_lidarcube_loss(radarcube, lidarcube, params, multiclass, class_weights=None):
    """
    ...
    if multiclass:
        ...
        # radarcube = radarcube.view(-1, 5, radarcube.shape[-2], radarcube.shape[-1])
        # lidarcube = lidarcube.view(-1, radarcube.shape[-2], radarcube.shape[-1])

        metric = torch.nn.CrossEntropyLoss(
            weight=class_weights,
            ignore_index=-1,  # 如果有需要忽略的标签值
            reduction='mean'
        )
    elif not params['bev']:
        batch_size = lidarcube.shape[0]
    ...
    return metric(radarcube, lidarcube.long())
...
def lidarpc_to_lidarcube_multiclass(lidar_pc, params):
    """
    It converts the lidar point cloud into a lidar cube using the same axis as the radar cube.
    The axis are not uniform.
...
def cube_to_pointcloud(cube, params, radar_cube, mode='radar', dop_fold_path=None, multiclass=False):
    """
    ...
    if mode == 'radar':
        ...
        doppler_indices = np.argmax(radar_cube[:, range_indices, azimuth_indices].cpu(), 0)
        # Convert to speed
...
def cube_to_pointcloud_multiclass(cube, params, radar_cube, mode='radar', dop_fold_path=None):
    """
    It finds those voxels that are 1 and transform the to Cartesian point cloud.
    ...
    if mode == 'radar':
        ...
        probs = torch.softmax(cube, dim=0)         # 形状: (5, 34, H, W)
        pred_classes = torch.argmax(probs, dim=0)  # 形状: (34, H, W)

        nonzero_indices = torch.nonzero(pred_classes)
        nonzero_indices = nonzero_indices.numpy()
...
def get_default_params():
    ...
    # Path to the dataset
    dataset_path = 'your/path/to/RaDelft/'

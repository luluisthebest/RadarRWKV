...
def RADIal_collate(batch):
    ...
    for radar_FFT, segmap,out_label,box_labels,image, sample_id in batch:
    ...
    return torch.stack(FFTs), torch.stack(encoded_label),torch.stack(segmaps),labels,torch.stack(images), sample_ids

def CreateDataLoaders(dataset,config=None,seed=0, dist=False):
    ...
    if(config['mode']=='random'):
        ...
        n_val = int(config['split'][1] * n_images)  + 1
        n_test = n_images - n_train - n_val - 1

        train_dataset, val_dataset, test_dataset = random_split(
            dataset, [n_train, n_val,n_test], generator=torch.Generator().manual_seed(seed))
        
        # train_dataset = train_dataset[:600]
        # val_dataset = val_dataset[:90]
        ...
        for seq in Sequences['Train']:
            idx =np.where(dataset.labels[:,14]==seq)[0]
            Train_indexes.append(dataset.labels[idx,0])
        Train_indexes = np.unique(np.concatenate(Train_indexes))
        '''
        Val_indexes = np.append(Val_indexes, Val_indexes[-1])
        val_ids = [dict_index_to_keys[k] for k in Val_indexes]
        test_ids = [dict_index_to_keys[k] for k in Test_indexes]
        #train_ids = [dict_index_to_keys[k] for k in Train_indexes]
        train_ids = np.setdiff1d(np.arange(len(dataset)),np.concatenate([val_ids,test_ids]))

        train_dataset = Subset(dataset,train_ids)#[:623])
        val_dataset = Subset(dataset,val_ids)#[:98])
        test_dataset = Subset(dataset,test_ids)#[:103])
        ...
        if dist:
            ...
            train_loader = DataLoader(train_dataset, sampler=train_sampler, batch_size=config['train']['batch_size'],
                                      num_workers=config['train']['num_workers'], pin_memory=True, collate_fn=RADIal_collate)
            val_sampler = torch.utils.data.RandomSampler(val_dataset)
            val_loader = DataLoader(val_dataset, sampler=val_sampler, batch_size=config['val']['batch_size'],
                                    num_workers=config['val']['num_workers'], pin_memory=True, collate_fn=RADIal_collate)            
            test_sampler = torch.utils.data.RandomSampler(test_dataset)
            test_loader = DataLoader(test_dataset, sampler=test_sampler, batch_size=config['test']['batch_size'], 
                                shuffle=False, num_workers=config['test']['num_workers'],
                                pin_memory=True, collate_fn=RADIal_collate)
